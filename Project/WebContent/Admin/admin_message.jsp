...
function del(i){
	...
	if (del==true){
		var dn = $("#value"+i).val();
		var message_id = $.trim(dn);
		location.href="AdminServlet?method=delete_message&message_id="+message_id;
	}
}
...
<div class="container">
  ...
  <div class="content">
   ...
   <tr>
    ...
    <table width="750" border="0">
    <tr>
    <c:forEach items="${page.list}" var="message" varStatus="idStatus">
     <tr>		
      <td>${idStatus.count}</td>
      <td><a href="${pageContext.request.contextPath }/Admin/AdminServlet?method=show_message&message_id=${message.message_id}">${message.message_topic}</a>
      <input type="hidden" id="value${idStatus.count }" value="${message.message_id }"></td>	
      <c:if test="${message.message_character==0 }">
      <td>[老师]${message.message_uploaduser}</td>
      </c:if>
      <c:if test="${message.message_character==1 }">
      <td>[管理员]${message.message_uploaduser}</td>
      </c:if>
      <c:if test="${message.message_character==2 }">
      <td>[学生]${message.message_uploaduser}</td>
      </c:if>
      <td>${message.message_uploadtime}</td>
      <td>点击量:${message.message_clicknum}</td>
