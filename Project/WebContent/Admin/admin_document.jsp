...
<script type="text/javascript">
	...
	function del(id){
		...
		var dn = $("#value" +id).val();
		var document_id = $.trim(dn);
		var am = $("#attachment"+id).val();
		var document_attachment = $.trim(am);
		if (del==true){
			location.href="AdminServlet?method=delete_document&document_id="+document_id +"&document_attachmet="+document_attachment;
			/*
   ...
   <tr>
    ...
    <c:forEach items="${page.list}" var="document" varStatus="idStatus">
     <tr>		
      ...
      <td><a href="${pageContext.request.contextPath }/Admin/AdminServlet?method=download_document&document_id=${document.document_id}">${document.document_name}</a>
      <input type="hidden" id="value${idStatus.count }" value="${document.document_id }">
      <input type="hidden" id="attachment${idStatus.count }" value="${document.document_attachment }">
      </td>	
      ...
      <td>点击量:${document.document_downloadnum}</td>
