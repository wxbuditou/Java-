...
<script type="text/javascript">
	...
	function del(id){
		...
		var dn = $("#value"+id).val();
		var news_id = $.trim(dn);
		location.href="AdminServlet?method=delete_news&news_id="+news_id;
...
<div class="container">
  ...
  <div class="content">
   ...
   <tr>
    ...
    <c:forEach items="${page.list}" var="news" varStatus="idStatus">
     <tr>		
      <td>${idStatus.count}</td>
      <td><a href="${pageContext.request.contextPath }/Admin/AdminServlet?method=show_news&news_id=${news.news_id}">${news.news_topic}</a>
      <input type="hidden" id="value${idStatus.count }" value="${news.news_id }"></td>	
      <c:if test="${news.news_character==0 }">
      <td>[老师]${news.news_uploaduser}</td>
      </c:if>
      <c:if test="${news.news_character==1 }">
      <td>[管理员]${news.news_uploaduser}</td>
      </c:if>
      <td>${news.news_uploadtime}</td>
